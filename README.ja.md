...
複数の独立した素朴な手描きステッカーを中央に集め、一つの主題を構成。格子整列や無作為な散布はしません。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 174 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-174` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-174 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-174-prompt.en.md)／[中国語](references/xxd-panel-174-prompt.zh-CN.md)を参照してください。
